...
-- Enable RLS on daily_digests table
ALTER TABLE daily_digests ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if any
DROP POLICY IF EXISTS "Allow public read access to daily digests" ON daily_digests;
DROP POLICY IF EXISTS "Allow service role to insert daily digests" ON daily_digests;
DROP POLICY IF EXISTS "Allow service role to update daily digests" ON daily_digests;
DROP POLICY IF EXISTS "Allow service role to delete daily digests" ON daily_digests;

-- Allow anyone to READ daily digests (for frontend display)
CREATE POLICY "Allow public read access to daily digests"
ON daily_digests
FOR SELECT
TO public
USING (true);
...
-- Allow service role to INSERT data (for GitHub Actions)
CREATE POLICY "Allow service role to insert daily digests"
ON daily_digests
FOR INSERT
TO service_role
WITH CHECK (true);
...
-- Allow service role to UPDATE data (for GitHub Actions)
CREATE POLICY "Allow service role to update daily digests"
ON daily_digests
FOR UPDATE
TO service_role
USING (true)
...
-- Allow service role to DELETE data (for cleanup)
CREATE POLICY "Allow service role to delete daily digests"
ON daily_digests
FOR DELETE
TO service_role
USING (true);
